...
def evaluate_transformer(args):
    ...
    with torch.no_grad():
        for i, (image, caps, caplens, allcaps, attrs, attrlens) in enumerate(tqdm(loader, desc="EVALUATING AT BEAM SIZE " + str(beam_size))):
            k = beam_size
            # Move to GPU device, if available
            image = image.to(device)  # [1, 3, 256, 256]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_folder', default="/mnt/data/facad/dataset",
    ...
    parser.add_argument('--checkpoint', default="./BEST_checkpoint_facad_1_cap_per_img_5_min_word_freq.pth.tar",
