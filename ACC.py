...
class LocalRNN(nn.Module):
    ...
    def forward(self, x):
        x = torch.nn.functional.glu(x, dim=-1) #16, 27, 256
        x = self.conv((x.transpose(1, 2))).transpose(1,2)
        nbatches, l, input_dim = x.shape #16, 27, 512
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, dec_inputs, enc_outputs, attr_out, dec_self_attn_mask, dec_enc_attn_mask, attr_dec_attn_mask):
        ...
        dec_outputs = torch.cat((dec_outputs_image, dec_outputs_attr), dim=2) #16, 27, 600
